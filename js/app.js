...
function initNotificationBell(profile, onUpdate) {
  ...
  // Every meaningful change in this app (attendance marked, marks uploaded,
  // leave approved/rejected, event published) already inserts a notification
  // row — so reacting to notification INSERTs is also our signal to refresh
  // the actual dashboard data, not just the notification panel. This is what
  // makes attendance %, marks, and leave status update live on screen instead
  // of only showing a toast that the data "changed somewhere."
...
// ---------- MESSAGING (Student <-> Incharge) ----------

async function fetchConversation(myId, otherId) {
  const { data, error } = await supabaseClient
    .from('messages')
    .select('*')
    .or(`and(sender_id.eq.${myId},recipient_id.eq.${otherId}),and(sender_id.eq.${otherId},recipient_id.eq.${myId})`)
    .order('created_at', { ascending: true });
  if (error) { console.error('fetchConversation error:', error.message); return []; }
  return data || [];
}

async function sendMessage(senderId, recipientId, bodyText) {
  const { error } = await supabaseClient.from('messages').insert({ sender_id: senderId, recipient_id: recipientId, body: bodyText });
  if (error) throw new Error(error.message);
}

// Realtime: fires whenever a message is inserted where I am sender or recipient.
// (Two subscriptions because postgres_changes filters only support one column check.)
function subscribeToMessages(profile, onNewMessage) {
  return supabaseClient
    .channel('messages-' + profile.id)
    .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages', filter: `recipient_id=eq.${profile.id}` }, (payload) => onNewMessage(payload.new))
    .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages', filter: `sender_id=eq.${profile.id}` }, (payload) => onNewMessage(payload.new))
    .subscribe();
}

function messageBubbleHTML(m, isMine) {
  return `
    <div style="display:flex;justify-content:${isMine ? 'flex-end' : 'flex-start'};margin-bottom:10px;">
      <div style="max-width:70%;background:${isMine ? 'var(--ink)' : 'var(--parchment)'};color:${isMine ? '#fff' : 'var(--ink)'};padding:10px 14px;border-radius:12px;${isMine ? 'border-bottom-right-radius:4px;' : 'border-bottom-left-radius:4px;'}">
        <div style="font-size:13px;">${m.body}</div>
        <div style="font-size:10px;opacity:.7;margin-top:4px;">${timeAgo(m.created_at)}</div>
      </div>
    </div>`;
}
...
async function fetchPresenceMap(studentIds) {
  ...
  const { data, error } = await supabaseClient.from('student_presence').select('*').in('student_id', studentIds);
  if (error) { console.error('fetchPresenceMap error:', error.message); return {}; }
  const map = {};
  (data || []).forEach(p => { map[p.student_id] = p; });
  return map;
}

// Realtime: any insert/update to student_presence in scope (RLS filters which
// rows this user is allowed to see, so no explicit department filter needed).
function subscribeToPresence(profile, onChange) {
  return supabaseClient
    .channel('presence-' + profile.department + '-' + (profile.section || 'dept'))
    .on('postgres_changes', { event: '*', schema: 'public', table: 'student_presence' }, () => onChange())
    .subscribe();
}
